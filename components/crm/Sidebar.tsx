"use client";
import {BarChart3,Box,ClipboardList,PackagePlus,Settings,Users,X} from "lucide-react";
export const navItems=[
 {id:"dashboard",label:"Tableau de bord",icon:BarChart3},{id:"new-order",label:"Nouvelle commande",icon:PackagePlus},
 {id:"orders",label:"Commandes",icon:ClipboardList},{id:"products",label:"Produits",icon:Box},
...
export function Sidebar({active,onChange,open,onClose}:{active:ScreenId;onChange:(id:ScreenId)=>void;open:boolean;onClose:()=>void}){
 return <><div className={"mobile-overlay "+(open?"visible":"")} onClick={onClose}/><aside className={"sidebar "+(open?"open":"")}>
  ...
  <nav><p className="nav-eyebrow">GESTION</p>{navItems.slice(0,4).map(({id,label,icon:Icon})=><button key={id} className={active===id?"active":""} onClick={()=>{onChange(id);onClose()}}><Icon size={19}/><span>{label}</span></button>)}
  <p className="nav-eyebrow second">ADMINISTRATION</p>{navItems.slice(4).map(({id,label,icon:Icon})=><button key={id} className={active===id?"active":""} onClick={()=>{onChange(id);onClose()}}><Icon size={19}/><span>{label}</span></button>)}</nav>
...
export function MobileNav({active,onChange}:{active:ScreenId;onChange:(id:ScreenId)=>void}){return <nav className="mobile-nav">{navItems.slice(0,5).map(({id,label,icon:Icon})=><button key={id} className={active===id?"active":""} onClick={()=>onChange(id)}><Icon size={20}/><span>{label==="Nouvelle commande"?"Commander":label==="Tableau de bord"?"Accueil":label}</span></button>)}</nav>}
